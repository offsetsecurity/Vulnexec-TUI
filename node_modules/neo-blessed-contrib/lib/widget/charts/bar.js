...
function Bar(options) {
	...
	if (this.options.showText === false) this.options.showText = false;
	else this.options.showText = true;
...
Bar.prototype.setData = function (bar) {
	...
	for (var i = 0; i < bar.data.length; i++) {
		...
		if (bar.data[i] > 0) {
			...
			if (this.options.barMultipleColor) {
				if (i <= this.options.barMultipleColor.length) {
					c.strokeStyle = this.options.barMultipleColor[i];
				} else {
					c.strokeStyle = this.options.barMultipleColor[0];
				}
			}
		...
		if (this.options.showText && this.options.showBarText)
			c.fillText(
				bar.data[i].toString() + this.options.suffix,
				x + 1,
